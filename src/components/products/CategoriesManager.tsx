  ...
  if (categoriasError) {
    return (
      <div className="py-4 text-red-500">
        Error al cargar categorías: {categoriasError.message}
      </div>
    );
  }
  ...
  return (
    ...
    >
      <TabsList>
        ...
        {isAdmin && <TabsTrigger value="add">Añadir Categoría</TabsTrigger>}
      </TabsList>
          ...
          <ul className="space-y-2">
            {categoriasOrganizadas.length > 0 ? (
                      ...
                      <ul className="pl-6 mt-1 space-y-1">
                        {category.subcategorias.map((subcategory) => (
                          <li key={subcategory.id}>
                            <Collapsible
                              open={expandedSubcategories.includes(
                                subcategory.id,
                              )}
                              onOpenChange={() =>
                            ...
                            >
                              <div className="flex items-center justify-between rounded-md hover:bg-muted p-2">
                                <div className="flex items-center">
                                  <CollapsibleTrigger asChild>
                                    ...
                                    >
                                      <ChevronRight
                                        className={`h-4 w-4 transition-transform ${expandedSubcategories.includes(subcategory.id) ? "rotate-90" : ""}`}
                                      />
                                    </Button>
                                  </CollapsibleTrigger>
                                  <span className="ml-2">
                              ...
                              <CollapsibleContent>
                                <ul className="pl-6 mt-1 space-y-1">
                                  {getNivel3Categories(subcategory.id).map(
                                    (nivel3) => (
                                      <li key={nivel3.id}>
                                        <div className="flex items-center justify-between rounded-md hover:bg-muted p-2">
                                          <div className="flex items-center">
                                          ...
                                          {isAdmin && (
                                            <Button
                                              ...
                                              size="sm"
                                              onClick={() =>
                                                handleDeleteCategory(nivel3.id)
                                              }
                                              className="h-7 w-7 p-0 hover:opacity-100"
                                            >
                                              <Trash className="h-4 w-4" />
              ...
              <p className="text-muted-foreground py-4">
                ...
                {isAdmin ? "Añade una nueva categoría." : ""}
              </p>
            )}
          </ul>
        </Card>
          ...
          <Card className="p-4">
            <h3 className="text-lg font-medium mb-4">
              {categoryFormData.parentId
                ? categoryFormData.nivel === 2
                  ? "Añadir Subcategoría"
                  : "Añadir Categoría Nivel 3"
                  ...
                  <SelectContent>
                    ...
                    {categorias
                      .filter(
                        (cat) =>
                          !cat.es_subcategoria ||
                          (cat.es_subcategoria && cat.nivel === 2),
                      )
                      .map((cat) => (
